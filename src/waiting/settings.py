...
def merge_hooks_into_settings(settings: dict) -> dict:
    ...
    # Add PostToolUse hook (for activity detection)
    if "PostToolUse" not in settings["hooks"]:
        settings["hooks"]["PostToolUse"] = []

    post_tool_use_hooks = settings["hooks"]["PostToolUse"]
    activity_hook_exists = _hook_exists(post_tool_use_hooks, f"{HOOK_NAME_ACTIVITY}.sh")
...
def remove_hooks_from_settings(settings: dict) -> dict:
    ...
    # Remove from PreToolUse (legacy cleanup)
