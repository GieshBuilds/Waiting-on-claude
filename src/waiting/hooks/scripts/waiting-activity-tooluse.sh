#!/bin/bash
# Hook for PostToolUse event
# Triggered after a tool completes (user has responded)
...
# Log the activity
echo "[$(date '+%Y-%m-%d %H:%M:%S')] User activity detected (PostToolUse). Stopped audio. Session: $SESSION_ID" >> "$LOG_FILE"
