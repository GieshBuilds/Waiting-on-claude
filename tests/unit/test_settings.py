...
class TestMergeHooksIntoSettings:
    ...
    def test_merge_adds_activity_hook(self):
        """Should add PostToolUse activity hook."""
        settings = {}
        paths = get_hook_paths()

        result = merge_hooks_into_settings(settings)

        post_tool_hooks = result["hooks"]["PostToolUse"]
        assert len(post_tool_hooks) > 0
        # Check that hook script path is included
        activity_hook_commands = [
            h["command"]
            for matcher in post_tool_hooks
            for h in matcher.get("hooks", [])
        ]
        assert any(
    ...
    def test_merge_idempotent(self):
        ...
        post_waiting_hooks = [
            h
            for h in result2["hooks"]["PostToolUse"]
            if h.get("matcher") == "*"
            and any(
        ...
        assert len(post_waiting_hooks) == 1
...
class TestRemoveHooksFromSettings:
    ...
    def test_remove_activity_hook(self):
        """Should remove PostToolUse activity hook."""
        settings = {}
        settings = merge_hooks_into_settings(settings)

        result = remove_hooks_from_settings(settings)

        # PostToolUse should be removed or empty
        if "PostToolUse" in result.get("hooks", {}):
...
class TestIsInstalled:
    ...
    def test_is_installed_false_when_posttooluse_missing(self, tmp_settings):
        """Should return False if PostToolUse hook missing."""
        settings = {}
        settings = merge_hooks_into_settings(settings)
        # Remove PostToolUse
