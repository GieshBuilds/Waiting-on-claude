...
class TestHookLifecycle:
    ...
    def test_activity_hook_script_contents(self, hook_manager_with_scripts):
        ...
        # Check for key functionality
        assert "PostToolUse" in content or "activity" in content.lower()
        assert "STOP_SIGNAL" in content
    ...
    def test_hook_registration_in_settings(self, hook_manager_with_scripts):
        ...
        post_hook_commands = [
            h["command"] for matcher in post_hooks for h in matcher.get("hooks", [])
        ]
        assert any("waiting-activity-tooluse.sh" in cmd for cmd in post_hook_commands)
